...
Sub GetYahooFinanceData()
    ...
    For Each cell In ws.Range(startCellSymbol + ":" + endCellSymbol).Cells
        ...
        If httpRequest.Status = 200 Then
            ...
            Dim dividendsByYear As Object
            Set dividendsByYear = CreateObject("Scripting.Dictionary")
            ...
            If result.Exists("events") Then
                If result("events").Exists("dividends") Then
                    ...
                    For Each key In dividends.Keys
                        ...
                        If dividendsByYear.Exists(dividendYear) Then
                            dividendsByYear(dividendYear) = dividendsByYear(dividendYear) + dividendAmount
                        Else
                            dividendsByYear.Add dividendYear, dividendAmount
                        End If
                    Next key
                End If
            End If
            ...
            For Each year2 In dividendsByYear.Keys
                Dim dateCell As Range
                Dim findString As String
                findString = "01.01." & year2
                Set dateCell = ws.Range(startCellDates + ":" + endCellDates).Find(What:=findString)
                If Not dateCell Is Nothing Then
                    ws.Cells(dateCell.row, cell.Column).Value = dividendsByYear(year2)
                End If
            Next year2
